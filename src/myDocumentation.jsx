        ...
        const fetchRequests = async () => {
            ...
            try {
                ...
                const sendRequest = async (token) => {
                    return await fetch(`${API_BASE}/api/myDocumentation`, {
                        method: "POST",
                        headers: {
                        ...
                        <div className="flex flex-col items-center gap-2" style={{ minWidth: '350px' }}>
                            ...
                            {requests.length === 0 ? (
                                <p className="text-center text-gray-600 text-lg font-semibold mt-10">
                                    No tienes Documentación cargada.
                                </p>
                            ) : (
                                requests.map((doc, index) => (
                                    <div
                                        key={index}
                                        className="w-full max-w-[800px] bg-white shadow-md rounded-lg px-5 py-4 border border-gray-200"
                                    >
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <p className="text-sm text-gray-500">
                                                    Cargado el {new Date(doc.created_at).toLocaleDateString()}
                                                </p>
                                            </td>
                                        </tr>
                                        <table className="w-full text-left text-gray-700">
                                            <tbody>
                                                <tr>
                                                    <td className="font-semibold w-1/4">Tipo:</td>
                                                    <td>{doc.tipo}</td>
                                                </tr>
                                                {doc.descripcion && (
                                                    <tr>
